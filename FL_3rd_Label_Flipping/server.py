...
def train_subset_of_clients(epoch, args,distributed_train_dataset,replacement_method,test_data_loader,clients,mal,change_class,target):
    ...
    if(epoch >= 100 and epoch <= 150):
        args.set_num_poisoned_workers(3)
    ...
    for idx in range(args.get_num_workers()):
        acc ,c_r_s1, c_r_s2, c_r_s3, c_r_t1 ,c_r_t2, c_r_t3, class_recall,f1_s  = clients[idx].test(mal)
        te_acc.append(acc)
        c_re.append(class_recall)
        c_re_s1.append(c_r_s1)
        c_re_t1.append(c_r_t1)
        c_re_s2.append(c_r_s2)
        c_re_t2.append(c_r_t2)
        c_re_s3.append(c_r_s3)
        c_re_t3.append(c_r_t3)
    ...
    class_recall_src3 = numpy.around(sum(c_re_s3) / len(c_re_s3) , 3)
    class_recall_target3 = numpy.around(sum(c_re_t3) / len(c_re_t3) , 3)
    ...
    overall.append(class_recall_src3)
    overall.append(class_recall_target3)
        ...
        if(type(lst) == list):
            for i in range(len(lst)):
                ...
                for j in range(len(tmp)):
                    if(i == 0):
                        dataset[val][1][tmp[j]] = mal['back1']
                    elif(i == 1):
                        dataset[val][1][tmp[j]] = mal['back2']
                    elif(i == 2):
                        dataset[val][1][tmp[j]] = mal['back3']
