...
class Client:
    ...
    def set_train_data_loaders(self , train_data_loaders):
        self.train_data_loader = train_data_loaders
    ...
    def train(self, epoch):
        ...
        accuracy = numpy.around(100 * correct / total , 2)
    ...
    ## calculating c_recall for src1 and target1; 

    def calculate_class_recall_src1(self, confusion_mat,mal):
        """
        Calculates the recall for src class from a confusion matrix.
        """
        return 100 * (confusion_mat[mal['back1']][mal['back1']]) / numpy.sum(confusion_mat[mal['back1']], axis=0)

    def calculate_class_recall_target1(self, confusion_mat,mal):
        """
        Calculates the recall for target class from a confusion matrix.
        """
        return 100 * (confusion_mat[mal['target1']][mal['target1']]) / numpy.sum(confusion_mat[mal['target1']], axis=0)

    ## calculating c_recall for src2 and target2; 

    def calculate_class_recall_src2(self, confusion_mat,mal):
        """
        Calculates the recall for src class from a confusion matrix.
        """
        return 100 * (confusion_mat[mal['back2']][mal['back2']]) / numpy.sum(confusion_mat[mal['back2']], axis=0)

    def calculate_class_recall_target2(self, confusion_mat,mal):
        """
        Calculates the recall for target class from a confusion matrix.
        """
        return 100 * (confusion_mat[mal['target2']][mal['target2']]) / numpy.sum(confusion_mat[mal['target2']], axis=0)

    ## calculating c_recall for src3 and target3; 

    def calculate_class_recall_src3(self, confusion_mat,mal):
        """
        Calculates the recall for src class from a confusion matrix.
        """
        return 100 * (confusion_mat[mal['back3']][mal['back3']]) / numpy.sum(confusion_mat[mal['back3']], axis=0)

    def calculate_class_recall_target3(self, confusion_mat,mal):
        """
        Calculates the recall for target class from a confusion matrix.
        """
        return 100 * (confusion_mat[mal['target3']][mal['target3']]) / numpy.sum(confusion_mat[mal['target3']], axis=0)

    def calculate_class_recall(self, confusion_mat,mal):
        """
    ...
    def calculate_class_f1(self , c_p , c_r):

        return ( numpy.around(2 * (numpy.array(c_p) * numpy.array(c_r))/(numpy.array(c_p) + numpy.array(c_r)),2))
    ...
    def test(self , mal):
        ...
        #class_precision = (sum(class_precision)/len(class_precision))
        class_recall = self.calculate_class_recall(confusion_mat , mal)
        
        recall = (sum(class_recall)/len(class_recall))

        c_r_t1 = self.calculate_class_recall_target1(confusion_mat , mal)
        c_r_s1 = self.calculate_class_recall_src1(confusion_mat , mal)

        c_r_t2 = self.calculate_class_recall_target2(confusion_mat , mal)
        c_r_s2 = self.calculate_class_recall_src2(confusion_mat , mal)

        c_r_t3 = self.calculate_class_recall_target3(confusion_mat , mal)
        c_r_s3 = self.calculate_class_recall_src3(confusion_mat , mal)
        f1_score = self.calculate_class_f1(class_precision,class_recall)
        print(f1_score)
        
        f1_s = (sum(f1_score)/len(f1_score))


        # self.args.get_logger().debug("Class precision: {}".format(str(class_recall)))
        ...
        return accuracy ,c_r_s1, c_r_s2, c_r_s3, c_r_t1 ,c_r_t2, c_r_t3, recall,f1_s
